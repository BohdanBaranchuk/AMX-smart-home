MODULE_NAME='HeatingFloor_macros'(DEV vdv_Macros, DEV Tablet[],
			sinteger neededTemp,integer fanCoilMode,
			integer but_fanCoils_temps[],integer but_fanCoils_ChangeMode[],
			integer but_selectRoom_air[], integer savedSelectedRoom[7],integer but_start,integer but_OnOff[])
...
{    PUSH:{
    LOCAL_VAR integer i
    send_string 0,"'+++++++++++++++++++'"
    send_string 0,"'++++++++length_array(but_selectRoom_air):',itoa(length_array(but_selectRoom_air)),'+++++++++++'"
	...
	{
	    send_string 0,"'------in------'"
	    send_string 0,"'------floorMode:',itoa(fanCoilMode),'------'"
	    send_string 0,"'------floorTemp:',itoa(neededTemp),'------'"
	    SELECT
	    {
		ACTIVE(fanCoilMode == 1):	// AUTO
		{
		    SEND_COMMAND vdv_Macros,"'FLOOR,',itoa(i),',AUTO,',itoa(neededTemp),',',$0A,$0D"
		    SEND_COMMAND 0,"'FLOOR,',itoa(i),',AUTO,',itoa(neededTemp),',',$0A,$0D"
		}
		ACTIVE(fanCoilMode == 2):	// OFF
		{
		    SEND_COMMAND vdv_Macros,"'FLOOR,',itoa(i),',OFF,',$0A,$0D"
		    SEND_COMMAND 0,"'FLOOR,',itoa(i),',OFF,',$0A,$0D"
		}
		ACTIVE(fanCoilMode == 3):	// ON
		{
		    SEND_COMMAND vdv_Macros,"'FLOOR,',itoa(i),',ON,',$0A,$0D"
		    SEND_COMMAND 0,"'FLOOR,',itoa(i),',ON,',$0A,$0D"
		}		
	    }
